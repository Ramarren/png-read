...
(defun finish-deinterlace (w h sub-images sub-arrays sub-heights)
  (let ((image-final (make-array (list w h 3) :initial-element 0)))
...
(defun decode-interlaced (data png-state)
  (let ((w (width png-state))
   ...
   (multiple-value-bind (sub-images sub-arrays sub-heights) (decode-subimages data png-state)
     (setf (image-data png-state) (finish-deinterlace w h sub-images sub-arrays sub-heights)
	   (width png-state) w
	   (height png-state) h))))