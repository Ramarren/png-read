    ...
    (let ((bda (max 1 (/ bd 8))))
	 ...
	 (iter (for scanline in-vector scanlines with-index k)
	       (iter (for x in-vector scanline from 1 with-index xi by bda)
		     (ecase bd
			  (1 (iter (for l from 0 below 8)
				   (for xi next (+ (* (1- xi) 8) l))
				   (until (>= xi w))
				   (set-image-slice-to-index xi k (ldb (byte 1 l) x) pal image-data)
				   (if t-map (setf (aref t-map xi k)
						   (aref transp (ldb (byte 1 l) x))))))
			  (2 (iter (for l from 0 below 4)
				   (for xi next (+ (* (1- xi) 4) l))
				   (until (>= xi w))
				   (set-image-slice-to-index xi k (ldb (byte 2 (* 2 l)) x) pal image-data)
				   (if t-map (setf (aref t-map xi k)
						   (aref transp (ldb (byte 2 (* 2 l)) x))))))
			  (4 (iter (for l from 0 below 2)
				   (for xi next (+ (* (1- xi) 2) l))
				   (until (>= xi w))
				   (set-image-slice-to-index xi k (ldb (byte 4 (* 4 l)) x) pal image-data)
				   (if t-map (setf (aref t-map xi k)
						   (aref transp (ldb (byte 4 (* 4 l)) x))))))
