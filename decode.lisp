	       ...
	       (iter (for x in-vector scanline from 1 with-index xi by bda)
		     (ecase bd
			  (1 (iter (for l from 0 below 8)
				   ...
				   (until (>= xii w))
				   (set-image-slice-to-index xii k (ldb (byte 1 l) x) pal image-data)
				   (if t-map (setf (aref t-map xii k)
						   (aref transp (ldb (byte 1 l) x))))))
			  (2 (iter (for l from 0 below 4)
				   (for xii next (+ (* (1- xi) 4) l))
				   (until (>= xii w))
				   (set-image-slice-to-index xii k (ldb (byte 2 (* 2 l)) x) pal image-data)
				   (if t-map (setf (aref t-map xii k)
						   (aref transp (ldb (byte 2 (* 2 l)) x))))))
			  (4 (iter (for l from 0 below 2)
				   (for xii next (+ (* (1- xi) 2) l))
				   (until (>= xii w))
				   (set-image-slice-to-index xii k (ldb (byte 4 (* 4 l)) x) pal image-data)
				   (if t-map (setf (aref t-map xii k)
